    ...
    while (1)
    {
        ...
        case OPTION_EDIT:
            ...
            if (strcasecmp(confirmBuffer, "Y") == 0)
            {
                // Perform the update
                if (currentChoice == 1)
                    editContact(&root, editedNode->contact->name, buffer, editedNode->contact->phoneNum, editedNode->contact->email);
                else if (currentChoice == 2)
                    editContact(&root, editedNode->contact->name, editedNode->contact->name, buffer, editedNode->contact->email);
                else if (currentChoice == 3)
                    editContact(&root, editedNode->contact->name, editedNode->contact->name, editedNode->contact->phoneNum, buffer);
